...
import gr.aueb.compression.gorilla.DecompressorPmcMr;
import gr.aueb.compression.gorilla.PmcMR;
 ...
 *
 */
public class CompressBenchmarkPmcMrFilter {

	private static final int MINIMUM_TOTAL_BLOCKS = 100_000;

	private static String[] FILENAMES = { 
//			"/home/panagiotis/timeseries/city_temperature-fixed.csv.gz",
//			"/home/panagiotis/timeseries/Stocks-UK.txt.gz", "/home/panagiotis/timeseries/Stocks-USA.txt.gz",
//			"/home/panagiotis/timeseries/Stocks-Germany.txt.gz", "/home/panagiotis/timeseries/basel-temp.csv.gz",
//			"/home/panagiotis/timeseries/basel-wind-speed.csv.gz", "/home/panagiotis/timeseries/air-sensor-data.csv.gz",
...
//			"/home/panagiotis/timeseries/foodprices.csv.gz", "/home/panagiotis/timeseries/poi-lat.csv.gz",
...
//			"/home/panagiotis/timeseries/SSD_HDD_benchmarks.csv.gz" 
	...
	@Test
	public void testPmcMr2() throws IOException {
		...
		for (String filename : FILENAMES) {
			...
			int timestamp = 0;
			Collection<Point> points = new ArrayList<>();
			while ((values = timeseriesFileReader.nextBlock()) != null && points.size() < 10_000_000) {
				for (Double value : values) {
					points.add(new Point(timestamp++, value.floatValue()));
				}
			...
			for (int logOfError = -7; logOfError < 12; logOfError++) {
				...
				for (Point point : points) {
					...
					maxPrecisionError = (precisionError > maxPrecisionError) ? precisionError : maxPrecisionError;
					assertEquals(point.getValue(), decompressedValue.floatValue(), ((float) Math.pow(2, logOfError)),
							"Value did not match");
				}
				System.out.println(String.format(
